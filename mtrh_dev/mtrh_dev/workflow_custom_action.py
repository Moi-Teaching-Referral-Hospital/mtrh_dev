...
@frappe.whitelist()	
def reevaluate_po(docname, bidder_declined=None):#CONTEXT IS PURCHASE ORDER
	doc = frappe.get_doc("Purchase Order", docname)	
	po_items = doc.get("items")
	if bidder_declined:
		raise_rfq(None, po_items)
		mark_rerouted_order(doc)
		doc.notify_update()
		return
	item_codes  = [x.get("item_code") for x in po_items]
	item_codes_q_str = '('+','.join("'{0}'".format(i) for i in item_codes)+')'
	awards = frappe.db.sql(f"""SELECT DISTINCT parent, default_supplier FROM\
		 `tabItem Default` WHERE parent in {item_codes_q_str}""", as_dict=True)
	awarded_items = [x.get("parent") for x in awards if  x.get("default_supplier")]
	if awarded_items and not bidder_declined:
		suppliers = [x.get("default_supplier") for x in awards]
		distinct_suppliers = list(dict.fromkeys(suppliers))
		item_category = doc.item_category
		for j in distinct_suppliers:
			supplier_items =[x.get("parent") for x in awards if x.default_supplier == j]
			supplier_items_in_table = [x for x in po_items if x.get("item_code") in supplier_items ]
			raise_order(j, supplier_items_in_table, item_category)
	unawarded_dict =[x for x in po_items if x.get("item_code") not in awarded_items]
	if unawarded_dict:
		raise_rfq(None, unawarded_dict)
	mark_rerouted_order(doc)
	doc.notify_update()
	return
def mark_rerouted_order(doc):
	status_to_update = "Re-Routed"
	if doc.docstatus == 1:
		doc.db_set("docstatus",2)
		status_to_update = "Canceled"
	doc.db_set("workflow_state",status_to_update)
def raise_order(supplier, supplier_items, item_category, material_request_number=None):
	try:
		...
		for item_dict in supplier_items:
			...
			po_doc.append('items',{
				...
				'material_request': material_request_number if material_request_number else None,
				'amount': amount,
				'net_amount': amount,
				'base_rate' : rate,
				'base_amount' : amount,
...
def raise_rfq(doc, items):
	item_codes = (x.get("item_code") for x in items)
	item_codes_q_str = '('+','.join("'{0}'".format(i) for i in item_codes)+')'
	item_category = doc.get("item_category") if doc else get_item_group(items[0].get("item_code"))
...
def append_items_to_rfq(sq_doc , items):
	...
	if "ORD" in str(items[0].get("parent")):
		material_request_number = str(items[0].get("material_request")) or None
...
def reroute_rfq_item(docname , item_ids):	
	try:
		...
		if doc.get("docstatus") =="1":
			doc.db_set("workflow_state","Cancelled and Routed to PO")
			doc.db_set("docstatus", 2)
			return
...
def auto_generate_purchase_order_by_material_request(doc,state):	
	...
	if item_category is None: return #FIXED AS IT WAS CAUSING AN ERROR FOR AUTOGENERATED MATERIAL REQUESTS.
...
def budget_balance(payload, document_date):
	...
	for itemrow in payload_to_use:
		department = ""
		project = ""
		if itemrow.get("department"): department = itemrow["department"]
		if itemrow.get("project"): project = itemrow["project"]
		...
		#1 GET BUDGET ID
		budget = ""
		if itemrow.get("department"):
			budget = frappe.db.get_value('Budget', {'department': department,"fiscal_year": fiscal_year, "docstatus":"1"}, 'name')
		elif itemrow.get("project"):
			budget = frappe.db.get_value('Budget', {'project': project,"fiscal_year": fiscal_year, "docstatus":"1"}, 'name')
		...
		itemrow["project"] = project
		itemrow["budget"] = budget
...
def get_item_award(item):
	current_award = frappe.db.sql(f"""SELECT name FROM `tabTender Quotation Award` WHERE item_code ='{item}'\
